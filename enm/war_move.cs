...
public class war_move : MonoBehaviour {
    ...
    void Start () {
        ...
        theirSpawner = GameObject.Find("warriorSpawner");
    }
            ...
            else
            {
                if (moving)
                {
                    ...
                    if (ifOut) //пока не истечет этот таймер, монстр должен вернуться "домой". Если он не успевает, он уничтожается
                    {
                        ...
                        if (timeToGetBack < 0f)
                        {
                            if (!GameObject.Find("eyeSpawner").GetComponent<PolygonCollider2D>().IsTouching(gameObject.GetComponent<BoxCollider2D>()))
                            {
                ...
                else
                {
                    ...
                    if (timeBetweenMoveCounter < 0f)
                    {
                        ...
                        if (GameObject.Find("eyeSpawner").GetComponent<PolygonCollider2D>().IsTouching(gameObject.GetComponent<BoxCollider2D>()) == false)
                        {
                            ifOut = true;
                        }
                        else { ifOut = false; }
